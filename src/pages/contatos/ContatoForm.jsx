...
export default function ContatoForm() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      base44.entities.Contato.filter({ id }).then(res => {
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Link to="/contatos"><Button variant="outline" className="gap-2"><ArrowLeft className="w-4 h-4" /> Voltar</Button></Link>}
      />
        ...
        <div className="bg-card rounded-xl border border-border p-6 space-y-5">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <Input value={form.nome} onChange={e => set('nome', e.target.value)} placeholder="Nome do contato" className="mt-1" />
            </div>
            <div>
              <Label>Cargo</Label>
            ...
            <div>
              ...
              <Input value={form.departamento} onChange={e => set('departamento', e.target.value)} placeholder="Ex: TI" className="mt-1" />
            </div>
            <div>
            ...
            <div>
              <Label>E-mail</Label>
              <Input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="contato@email.com" className="mt-1" />
            </div>
            <div>
              ...
              <Select value={form.influencia_compra} onValueChange={v => set('influencia_compra', v)}>
                ...
                <SelectContent>
                  {['Decisor', 'Influenciador', 'Técnico', 'Usuário Final', 'Bloqueador'].map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="sm:col-span-2">
              <Label>LinkedIn</Label>
              <Input value={form.linkedin} onChange={e => set('linkedin', e.target.value)} placeholder="https://linkedin.com/in/..." className="mt-1" />
            </div>
            <div className="sm:col-span-2">
              <Label>Observações</Label>
              <Textarea value={form.notas} onChange={e => set('notas', e.target.value)} placeholder="Notas sobre o contato..." className="mt-1 resize-none" rows={3} />
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 mt-4">
          <Link to="/contatos"><Button variant="outline" type="button">Cancelar</Button></Link>
